...
function get_param() {
    return {
        ...
        //Imagem: $('#txt_imagem').prop('files')[0] // deu ruin por enquanto aqui pos no ato de altera a imagem outro item vai traz o nome do anterior
    }
}

$(document)
    .ready(function () {//utilização jquery mask para que o usuario so possa digitar numeros
    ...
    .on('click', '.btn-exibir-imagem', function () {
        var nome_imagem = $(this).closest('tr').attr('data-imagem'),
            ...
            template_imagem = $('#template_imagem'),
        ...
        bootbox.dialog({
            ...
            .on('hidden.bs.modal', function () {
                modal_imagem.hide().appendTo('boby');
            });
    })
    .on('click', '.btn-sair_imagem', function () {// so para recaregar a proxima imagem, no ato de fechar o modal ele recarega a pagina e assim poder 
                                                  //ser carregada outra imagem quando clicar novamente no botao da imagem
         location.reload(true);
    });
