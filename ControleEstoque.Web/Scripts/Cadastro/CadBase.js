...
function marcar_ordenacao_campo(coluna) {//39 video
...
function formata_mensagem_aviso(mensagens) {
    //var ret = '';
    ...
    //esse codigo a baixo reprecenta o mesmo do que esta acima, so que usando o Mustache^
...
function abrir_form(dados) {
    ...
    bootbox.dialog({
        ...
        className: 'dialog',  //essa forma ele pega a class e atribio a largura para o modal
    })
...
function criar_linha_grid(dados) {
    ...
    //esse codigo a baixo reprecenta o mesmo do que esta acima, mas teve a participação do mustache no index do pais como exep, no local dos script,
...
function salvar_ok(response, param) {
    if (response.Resultado == "OK") {
        ...
        $('#msg_mensagem_aviso').html(formata_mensagem_aviso(response.Mensagens));
        $('#msg_aviso').show();
